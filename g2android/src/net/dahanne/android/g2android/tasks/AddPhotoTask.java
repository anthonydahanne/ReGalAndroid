...
//				imageFile = UriUtils.createFileFromUri(openInputStream,
//						mimeType);
			}
			if (mustLogIn) {
			...
			imageCreatedName = G2ConnectionUtils.getInstance()
					.sendImageToGallery(galleryUrl, albumName, imageFile,imageName);
			//we keep the image from the camera
//			if((File) parameters[5]==null){
//				imageFile.delete();
//			}
		} catch (Exception e) {
...
//
//			}
			ShowUtils.getInstance().toastImageSuccessfullyAdded(activity,imageName);
		}
